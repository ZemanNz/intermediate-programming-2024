...
bool can_make(const std::string &goal, const std::set<std::string> &start, const std::map<std::set<std::string>, std::string> &recipes) {
    if(start.contains(goal)){
        return true;
    }
    // Projde všechny recepty, které produkují požadovaný cíl
    for (auto const & [ingredients, output] : recipes) {
        if (output == goal) {
            bool all_possible = true;
            // Pro každou ingredienci z receptu se rekurzivně ověří, zda ji lze získat
            for (auto const & ingredient : ingredients) {
                if (!can_make(ingredient, start, recipes)) {
                    all_possible = false;
                    break;
                }
            }
